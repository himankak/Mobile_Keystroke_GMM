...
#the training(), testing() and evaluateEER() function change, rest all is same.            
    ...
    def training(self):
        self.gmm = GaussianMixture(n_components = 3, covariance_type = 'full', verbose = False, random_state = 150)
...
#            data.columns.str.startswith('RRT')
            ...
            self.test_imposter = imposter_data.groupby("subject"). \
                                 head(3).loc[:, "FA.1":"FA.1"]
# data.columns.str.startswith('RRT')
            self.training()
            self.testing()
            eers.append(evaluateEERGMM(self.user_scores_new, self.imposter_scores_new))
        print(eers)
        return np.mean(eers)
